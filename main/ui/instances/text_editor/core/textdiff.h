...
public:
  ...
  inline void SetShowWhitespacesEnabled(bool value) {
    diff.config.showSpaces = value;
    diff.config.showTabs = value;
  }
  inline bool IsShowWhitespacesEnabled() const {
    return diff.config.showSpaces && diff.config.showTabs;
  }
  ...
  inline void SetShowScrollbarMiniMapEnabled(bool value) {
    diff.config.showScrollbarMiniMap = value;
  }
  inline bool IsShowScrollbarMiniMapEnabled() const {
    return diff.config.showScrollbarMiniMap;
  }
  inline void SetLanguage(const TextEditorInternal::Language *language) {
    diff.config.language = language;
  }
  inline const TextEditorInternal::Language *GetLanguage() const {
    return diff.config.language;
  };
  inline void SetColors(ImU32 ac, ImU32 dc) {
    diff.addedColor = ac;
    diff.deletedColor = dc;
  }

  inline void SetPalette(const TextEditorInternal::Palette &newPalette) {
    diff.paletteBase = newPalette;
    diff.paletteAlpha = -1.0f;
  }
  inline const TextEditorInternal::Palette &GetPalette() const {
    return diff.paletteBase;
  }
...
private:
  ...
  enum class DiffType { common, added, deleted };

  class LineState {
  public:
    LineState(size_t leftLine, size_t rightLine, DiffType type)
        : leftLine(leftLine), rightLine(rightLine), type(type) {}
    size_t leftLine;
    size_t rightLine;
    DiffType type;
  };
  ...
  struct Diff {
    ...
    TextEditorInternal::Config config;
    bool focusOnDiff = false;

    // the two documents being compared
    TextEditorInternal::Document leftDocument;
    TextEditorInternal::Document rightDocument;

    // coloring supports
    TextEditorInternal::Colorizer leftColorizer;
    TextEditorInternal::Colorizer rightColorizer;
    ...
    TextEditorInternal::Palette paletteBase;
    TextEditorInternal::Palette palette;
    float paletteAlpha;
  } diff;
  ...
  struct IntegratedView {
    ...
    struct Row {
      ...
      Row(DiffType type, size_t leftLine, size_t rightLine, size_t section,
          size_t columns)
          : type(type), leftLine(leftLine), rightLine(rightLine),
    ...
    // layout support
    TextEditorInternal::LineFold leftLineFold;
    TextEditorInternal::LineFold rightLineFold;
    TextEditorInternal::TypeSetter leftTypeSetter;
  ...
  struct SideBySideView {
    ...
    struct Row {
      ...
      Row(DiffType type, size_t leftLine, size_t leftSection, size_t rightLine,
          size_t rightSection, size_t columns)
          : type(type), leftLine(leftLine), leftSection(leftSection),
            rightLine(rightLine), rightSection(rightSection), columns(columns) {
      }
