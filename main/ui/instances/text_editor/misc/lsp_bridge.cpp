//	TextEditorInternal - A syntax highlighting text editor for ImGui
...
bool LspBridge::Start(const std::string &rootPath,
  ...
  try {
    ...
    std::stringstream ss;
    ss << "Can't start language server[" << executable
       << "], error: " << e.what();
    errorMessage = ss.str();
    return false;
  }
...
void LspBridge::OpenDocument(const std::string &path,
                             TextEditorInternal &editor, int options) {
  if (IsRunning()) {
...
LspBridge::Document::Document(LspBridge &bridge, const std::string &path,
                              TextEditorInternal &editor, int options)
  ...
  bridge.messageHandler
      ->sendNotification<lsp::notifications::TextDocument_DidOpen>(
          {.textDocument = {.uri = lsp::DocumentUri::fromPath(path),
  ...
  contentChanges.emplace_back(
      lsp::TextDocumentContentChangeEvent_Text({.text = editor.GetText()}));
  ...
  bridge.messageHandler
      ->sendNotification<lsp::notifications::TextDocument_DidChange>(
          std::move(params));

  // track changes in document from now on and pass them to the language server
  editor.SetTransactionCallback([&](const std::vector<
                                    TextEditorInternal::Change> &changes) {
    lsp::Array<lsp::TextDocumentContentChangeEvent> contentChanges;

    for (auto &change : changes) {
      if (change.insert) {
        contentChanges.emplace_back(
            lsp::TextDocumentContentChangeEvent_Range_Text(
                {.range = lsp::Range(
                     {.start = lsp::Position(
                          {.line = static_cast<lsp::uint>(change.start.line),
                           .character =
                               static_cast<lsp::uint>(change.start.index)}),
                      .end = lsp::Position(
                          {.line = static_cast<lsp::uint>(change.start.line),
                           .character =
                               static_cast<lsp::uint>(change.start.index)})}),
                     ...
                     {.start = lsp::Position(
                      ...
                      .end = lsp::Position(
                          {.line = static_cast<lsp::uint>(change.end.line),
                           .character =
                               static_cast<lsp::uint>(change.end.index)})}),
                 .text = ""}));
      }
    }
  ...
  if (options & autocomplete) {
    TextEditorInternal::AutoCompleteConfig config;

    config.callback = [&](TextEditorInternal::AutoCompleteState &state) {
      bridge.messageHandler->sendRequest<
          lsp::requests::TextDocument_Completion>(
          {{.textDocument = {.uri = lsp::DocumentUri::fromPath(path)},
            .position = lsp::Position{.line = static_cast<lsp::uint>(
                                          state.searchTermEnd.line),
                                      .character = static_cast<lsp::uint>(
                                          state.searchTermEnd.index)}}},
...
LspBridge::Document::~Document() {
  ...
  bridge.messageHandler
      ->sendNotification<lsp::notifications::TextDocument_DidClose>(
          {.textDocument = {.uri = lsp::DocumentUri::fromPath(path)}});
...
void LspBridge::Document::update() {
  ...
  if (editor.IsMousePosOverGlyph(ImGui::GetMousePos())) {
    ...
    if (docPos != mouseDocPos) {
      ...
      if (options & showSignature) {
        bridge.messageHandler
            ->sendRequest<lsp::requests::TextDocument_SignatureHelp>(
                {{.textDocument = {.uri = lsp::DocumentUri::fromPath(path)},
                  .position =
                      lsp::Position{.line = static_cast<lsp::uint>(docPos.line),
                                    .character =
                                        static_cast<lsp::uint>(docPos.index)}}},
                [&](const lsp::requests::TextDocument_SignatureHelp::Result
                        &result) {
                  if (!result.isNull()) {
                    signatures.clear();
        ...
        bridge.messageHandler->sendRequest<lsp::requests::TextDocument_Hover>(
            ...
            [this](const lsp::requests::TextDocument_Hover::Result &result) {
              if (!result.isNull()) {
                ...
                if (std::holds_alternative<lsp::MarkupContent>(
                        result->contents)) {
  ...
  if (options & showHoverHelp) {
    ...
    if (hoverMessage.size() &&
        editor.IsMousePosOverGlyph(ImGui::GetMousePos())) {
      editor.SetTextHoverCallback([&](TextEditorInternal::PopupData) {
