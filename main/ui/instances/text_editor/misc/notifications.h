...
//	Notifications
//
//	This class provides a "toast" notification system. Customers only have
// to Add 	notification by providing a type, message and a dismissal time
// in microseconds 	(the default is 400ms or 4 seconds).
//
//	Notifications show up as a stack growing from the bottom to the top.
// Older 	messages will fade out and collapse. The user simply calls the
// Render method 	every frame with a bottom left coordinate in viewport
// space and this will update 	the lifecycle of each notification an render the
// stack.
...
public:
  ...
  inline void Add(Type type, const std::string &message,
                  int dismissTime = 4000) {
    notifications.emplace_back(type, message, dismissTime);
  }

  // render notifications as a stack (position is bottom-right of stack in
  // viewport space)
...
private:
  ...
  struct Notification {
    // constructor
    Notification(Type type, const std::string &message, int dismissTime)
        : message(message) {
      std::stringstream ss;
      ss << "Notification" << id++;
      name = ss.str();
    ...
    inline void update() {
      ...
      if (phase == Phase::fadeIn) {
        alpha = std::chrono::duration_cast<dmilliseconds>(now - fadeInStart) /
                fadeInDuration;
        ...
        alpha = 1.0f -
                (std::chrono::duration_cast<dmilliseconds>(now - fadeOutStart) /
                 fadeOutDuration);
      ...
      if (phase == Phase::ghost) {
        ghostHeight =
            (1.0f -
             (std::chrono::duration_cast<dmilliseconds>(now - ghostStart) /
              ghostDuration)) *
            height;
      }
    }
    ...
    inline float render(ImVec2 pos, float offset) {
      if (phase == Notification::Phase::ghost) {
        ...
        ImGui::SetNextWindowPos(ImVec2(pos.x, pos.y - offset), ImGuiCond_Always,
                                ImVec2(1.0f, 1.0f));
        ...
        auto buttonWidth = ImGui::CalcTextSize("x").x +
    ...
    enum class Phase { fadeIn, wait, fadeOut, ghost, expired };
