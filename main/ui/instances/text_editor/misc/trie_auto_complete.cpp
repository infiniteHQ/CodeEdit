...
void TrieAutoComplete::Connect(TextEditorInternal *newEditor) {
  ...
  config.callback = [this](TextEditorInternal::AutoCompleteState &state) {
  ...
  editor->SetLanguageChangeCallback([this]() { buildTrie(); });

  // enable change tracking
  // we don't track every keystroke, callbacks can be delayed up to 2000
  // milliseconds if you want live tracking, change the 2000 to 0 (performance
  // hit will be minimal for small documents)
  editor->SetChangeCallback([this]() { buildTrie(); }, 2000);
}
...
void TrieAutoComplete::buildTrie() {
  ...
  if (language) {
    for (const auto &word : language->keywords) {
      trie.insert(word);
    }
    for (const auto &word : language->declarations) {
      trie.insert(word);
    }
    for (const auto &word : language->identifiers) {
      trie.insert(word);
    }
  }

  // add all identifiers in current document
  editor->IterateIdentifiers(
      [this](const std::string &identifier) { trie.insert(identifier); });
}
...
void TrieAutoComplete::Trie::insert(const std::string_view &word) {
  auto node = root.get();
  auto end = word.end();
  auto i = TextEditorInternal::CodePoint::skipBOM(word.begin(), end);

  while (i < end) {
    ImWchar codepoint;
    i = TextEditorInternal::CodePoint::read(i, end, &codepoint);
...
void TrieAutoComplete::Trie::findSuggestions(
    std::vector<std::string> &suggestions, const std::string_view &searchTerm,
  ...
  if (searchTerm.size() != 0) {
    ...
    auto i = TextEditorInternal::CodePoint::skipBOM(searchTerm.begin(), end);
...
void TrieAutoComplete::Trie::evaluateNode(const Node *node, size_t index,
                                          size_t cost, size_t skip) {
  // see if that is one of our children (check both lower and uppercase matches)
  ImWchar codepointLower =
      TextEditorInternal::CodePoint::toLower(searchCodepoints[index]);
  ...
  ImWchar codepointUpper =
      TextEditorInternal::CodePoint::toUpper(searchCodepoints[index]);
